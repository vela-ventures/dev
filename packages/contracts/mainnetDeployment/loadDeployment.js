const configParams = require("./deploymentParams.load.js");
...
const { dec } = th;
const LoadDeploymentHelper = require("../utils/loadDeploymentHelpers.js");
const toBigNum = ethers.BigNumber.from;

async function loadDeploy(configParams) {
  const date = new Date();
  console.log(date.toUTCString());
  const deployerWallet = (await ethers.getSigners())[0];
  // const account2Wallet = (await ethers.getSigners())[1]
  const ldh = new LoadDeploymentHelper(configParams, deployerWallet);
  const gasPrice = configParams.GAS_PRICE;

  const deploymentState = ldh.loadPreviousDeployment();
  ...
  const liquityCore = await ldh.deployLiquityCoreMainnet(deploymentState);
  await ldh.logContractObjects(liquityCore);
  ...
  if (LUSDWETHPairAddr == th.ZERO_ADDRESS) {
    ...
    await ldh.sendAndWaitForTransaction(
  ...
  // Deploy Unipool
  const unipool = await ldh.deployUnipoolMainnet(deploymentState);

  // Deploy LQTY Contracts
  const LQTYContracts = await ldh.deployLQTYContractsMainnet(
  ...
  await ldh.connectCoreContractsMainnet(liquityCore, LQTYContracts);
  await ldh.connectLQTYContractsMainnet(LQTYContracts);
  await ldh.connectLQTYContractsToCoreMainnet(LQTYContracts, liquityCore);

  // Deploy a read-only multi-trove getter
  const multiTroveGetter = await ldh.deployMultiTroveGetterMainnet(liquityCore, deploymentState);
  ...
  await ldh.connectUnipoolMainnet(unipool, LQTYContracts, LUSDWETHPairAddr, LPRewardsDuration);

  // Log LQTY and Unipool addresses
  await ldh.logContractObjects(LQTYContracts);
  ...
  for (const [investor, investorAddr] of Object.entries(configParams.beneficiaries)) {
    ...
    if (deploymentState[investor] && deploymentState[investor].address) {
      ...
      const txReceipt = await ldh.sendAndWaitForTransaction(
  ...
  // await ldh.sendAndWaitForTransaction(liquityCore.borrowerOperations.repayLUSD(dec(800, 18), th.ZERO_ADDRESS, th.ZERO_ADDRESS, {gasPrice, gasLimit: 1000000}))
  ...
  // await ldh.sendAndWaitForTransaction(liquityCore.borrowerOperations.addColl(th.ZERO_ADDRESS, th.ZERO_ADDRESS, {value: dec(2, 'ether'), gasPrice, gasLimit: 1000000}))
  ...
  //   await ldh.sendAndWaitForTransaction(unipool.stake(1, { gasPrice }))
  // } else {
  ...
  // // Provide to SP
  // await ldh.sendAndWaitForTransaction(liquityCore.stabilityPool.provideToSP(dec(15, 18), th.ZERO_ADDRESS, { gasPrice, gasLimit: 400000 }))
  ...
  // // Withdraw from SP
  // // await ldh.sendAndWaitForTransaction(liquityCore.stabilityPool.withdrawFromSP(dec(1000, 18), { gasPrice, gasLimit: 400000 }))
  ...
  // // await ldh.sendAndWaitForTransaction(LQTYContracts.lqtyToken.transfer(account2LockupContract.address, dec(10, 18), { gasPrice }))
  ...
  // await ldh.sendAndWaitForTransaction(account2LockupContract.withdrawLQTY({ gasPrice, gasLimit: 1000000 }))
  ...
  // // stake 13 LQTY
  // await ldh.sendAndWaitForTransaction(LQTYContracts.lqtyStaking.stake(dec(13, 18), { gasPrice, gasLimit: 1000000 }))
  ...
  //   await ldh.sendAndWaitForTransaction(borrowerOpsAcct2.openTrove(th._100pct, _2kLUSDWithdrawal, th.ZERO_ADDRESS, th.ZERO_ADDRESS, { value: _1pt5_ETHcoll, gasPrice, gasLimit: 1000000 }))
  // } else {
  ...
  // // Deployer withdraws staking gains
  // await ldh.sendAndWaitForTransaction(LQTYContracts.lqtyStaking.unstake(0, { gasPrice, gasLimit: 1000000 }))
  ...
  // TCR
  const TCR = await liquityCore.troveManager.getTCR(2);
  console.log(`TCR: ${TCR}`);
  ...
  const S = await liquityCore.stabilityPool.scaleToSum(currentScale);
  const G = await liquityCore.stabilityPool.scaleToG(currentScale);
  th.logBN("Product P", P);
  // th.logBN("Current epoch", currentEpoch);
  th.logBN("Current scale", currentScale);
  th.logBN("Sum S, at current scale", S);
  th.logBN("Sum G, at current scale", G);
